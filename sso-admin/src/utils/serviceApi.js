...
/**
 * Builds a request function for the SSO service. Every response is
 * unwrapped from the shared `ApiResponse<T>` envelope
 * (`{success, code, message, data}`) so callers just get `data` back
 * directly and a thrown `Error(message)` on failure.
 *
