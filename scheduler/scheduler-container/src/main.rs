...
use scheduler_application::config::AppConfig::create_app_state;
use scheduler_application::config::DatabaseConfig::DatabaseConfig;
use scheduler_application::engine::SchedulerEngine;
use scheduler_application::rest::router::ApiRouter::create_router;
use scheduler_application::openapi::ApiDoc;
use utoipa::OpenApi;
...
async fn main() {
    ...
    // Start the background engine that fires due schedulers on their cron schedule
    SchedulerEngine::start(state.clone());
    ...
    // Browser-based clients hit this from a different origin/port — without
    // this, the browser blocks every request at the CORS preflight before it
