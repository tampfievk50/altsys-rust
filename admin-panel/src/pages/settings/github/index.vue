<script setup>
definePage({
  meta: { navActiveLink: 'settings-github' },
})

// GitHub repository connections are stored as Tool rows with tool_type
// "github" — the Tool Registry has no dedicated schema per type, so this
// page owns the mapping between named fields and the opaque `config` JSON
// string (repository, default_branch, credential_id).
const TOOL_TYPE = 'github'
...
const headers = [
  ...
  { title: 'Default Branch', key: 'default_branch' },
...
const emptyForm = () => ({ name: '', description: '', repository: '', defaultBranch: 'main', credentialId: null, isEnabled: true })
...
const buildConfig = () => JSON.stringify({
  ...
  default_branch: form.value.defaultBranch || 'main',
...
<template>
  <VCard title="GitHub Repositories">
    <template #append>
    ...
    <p class="text-body-2 text-medium-emphasis mx-4 mb-2">
      Connects the pipeline to a GitHub repository for branch/PR operations. The credential should hold a personal access token or GitHub App token with repo access.
    </p>
  ...
  <VDialog
    ...
    max-width="560"
  >
    <VCard :title="editingId ? 'Edit GitHub Repository' : 'New GitHub Repository'">
      <VCardText>
        <VAlert
          v-if="formError"
          type="error"
          ...
          <VRow>
            ...
            >
              <RemoteSelect
                ...
                label="Credential (access token)"
                :fetch-options="fetchCredentials"
                :item-title="c => `${c.name} (${c.provider})`"
                :rules="[requiredValidator]"
                empty-text="No GitHub credentials for this tenant yet — create one in Settings → Credentials with provider GitHub."
              />
            </VCol>
            <VCol cols="12">
              <AppTextarea
  ...
  <ConfirmDialog
    v-model="confirmOpen"
    title="Delete this repository connection?"
